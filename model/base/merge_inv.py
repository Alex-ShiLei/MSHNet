...
class merge(nn.Module):
    def __init__(self, shot, nfeatures=[2048 * 2, 1024 * 2, 512 * 2], nsimlairy=[3, 6, 4],criter=None):
        ...
        for num in nsimlairy:
            self.qs_layer_corrConv.append(BasicConv1x1(num, 128))
        ...
        self.conv4 = MergeConv1(256, 128, 256)
        self.conv3 = MergeConv1(256, 128, 256)
        self.conv2 = MergeConv1(256, 128, 256)
    ...
    def forward(self, qur_sups, sims,gt=None):
        qs_sim = []
        corr_sim = []
        ...
        for ly in range(len(self.nsimlairy)):
            for s in range(1,self.shot):
                corr_lySim[ly]+=(corr_sim[s][ly])
                qs_lySim[ly]+=(qs_sim[s][ly])
        x4 = self.conv4(self.simShotConv4(corr_lySim[2]/self.shot),
                        self.sqShotConv4(qs_lySim[2]/self.shot))
        x3 = self.conv3(self.simShotConv3(corr_lySim[1]/self.shot),
                        self.sqShotConv3(qs_lySim[1]/self.shot))
        x2 = self.conv2(self.simShotConv2(corr_lySim[0]/self.shot),
                        self.sqShotConv2(qs_lySim[0]/self.shot))
        x3_1 = F.interpolate(x3, x4.size()[-2:], mode='bilinear', align_corners=True)
        x43 = self.conv43(x4, x3_1)
        x2_1 = F.interpolate(x2, x43.size()[-2:], mode='bilinear', align_corners=True)
        x432 = self.conv432(x43, x2_1)
